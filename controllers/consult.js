...
const consultPOST = async (req = request, res = response) => {
  try {
    ...
    let patient = await Patient.findOneAndUpdate(
      { dni: dni },
      { $push: { pressure: pressure, weight: weight } }
    );
...
const agregarDiagnosticoPUT = async (req = request, res = response) => {
  try {
    ...
    const consult = await Consult.findByIdAndUpdate(id, {
      diagnosis: diagnosis,
      medicines: medicines,
      test: test,
    });
...
const consultPUT = async (req = request, res = response) => {
  try {
    ...
    const consulta = await Consult.findByIdAndUpdate(id, {
      " symptoms": symptoms,
      diagnosis: diagnosis,
      medicines: medicines,
      test: test,
...
const consultDELETE = async (req = request, res = response) => {
  ...
  let patient = await Patient.findOneAndUpdate(
    { dni: consulta.patient.dni },
    { $pull: { pressure: consulta.pressure, weight: consulta.weight } }
  );
